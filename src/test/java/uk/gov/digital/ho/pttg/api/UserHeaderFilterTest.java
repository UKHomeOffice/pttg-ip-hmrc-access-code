...
public class UserHeaderFilterTest {
    ...
    public void shouldUseCollaborators() throws IOException, ServletException {
        ...
        when(mockHttpServletRequest.getHeader(USER_ID_HEADER)).thenReturn(null);
        when(mockMdcUtility.generateDefaultUserId()).thenReturn("never seen");

        userHeaderFilter.doFilter(mockHttpServletRequest, mockServletResponse, mockFilterChain);

        verify(mockFilterChain).doFilter(mockHttpServletRequest, mockServletResponse);
        verify(mockMdcUtility).generateDefaultUserId();
    }
    ...
    @Test
    public void shouldAddToDefaultDataToMdcBeforeProceeding() throws IOException, ServletException {
        when(mockHttpServletRequest.getHeader(USER_ID_HEADER)).thenReturn(null);
        when(mockMdcUtility.generateDefaultUserId()).thenReturn("default data should go in the MDC");

        assertThat(MDC.get(USER_ID_HEADER))
                .as("MDC needs to be empty before the test")
                .isEqualTo(null);

        FilterChain stubFilterChain = (request, response) -> assertThat(MDC.get(USER_ID_HEADER)).isEqualTo("default data should go in the MDC");
